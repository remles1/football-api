...
def placeholder_func(n: int) -> int:
    return n * 2
...
async def calc_odds(data: str) -> dict:
    ...
    return_dict = {"home_team": float(np.sum(np.tril(decyzja, -1))), "draw": float(np.sum(np.diag(decyzja))),  "away_team": float(np.sum(np.triu(decyzja, 1)))}
    return return_dict
...
async def main(retries: int = 5, delay: int = 5) -> None:
    ...
    # Perform connection
    # connection = await connect("amqp://user:password@rabbitmq/")
    # Creating a channel
